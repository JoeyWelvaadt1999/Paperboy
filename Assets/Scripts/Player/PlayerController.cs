...
//			Speed = 20F;		
//			GetComponent<Rigidbody2D>.velocity = new Vector2(Input.acceleration.x * Speed, 0);
			...
			Vector2 tpos = transform.position;
						...
						tpos.x += Input.GetAxis("Horizontal") * 0.2f;
						tpos.y += Input.GetAxis("Vertical") * 0.1f;
						tpos.x = Mathf.Clamp (tpos.x, -2.2f, 2.2f);
						tpos.y = Mathf.Clamp (tpos.y, -3f, 3f);
...
//			#endif
	...
	void OnTriggerEnter2D(Collider2D Coll)
	{
		...
		IEnumerator LostPickup() {
				Color rc = GetComponent<SpriteRenderer> ().material.color;
				for (int i = 0; i < 4; i++) {
						if (rc.a == 0) {
								rc.a = 1;
								GetComponent<SpriteRenderer> ().material.color = rc;
						} else {
								rc.a = 0;
								GetComponent<SpriteRenderer> ().material.color = rc;
						}
